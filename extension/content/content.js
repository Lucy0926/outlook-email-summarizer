...
状态(用用户消息中的现在时间判断，精确到小时分钟):
- 截止时间已过(含同一天已过了该时刻)→【已结束】, status="已结束"
- 活动已开始但未结束→【进行中】, status="进行中"
- 截止时间在7天内且未过→【即将截止】, status="即将截止"
- 例: 现在6月14日19:19，活动截止"6月14日上午10:00"→已结束

每项必须提取: 截止时间、活动地点(如有)，填入deadline和location字段。
