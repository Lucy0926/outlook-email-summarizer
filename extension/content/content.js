...
// ========== System Prompt ==========
const SYSTEM_PROMPT = `⚠️ 你必须只输出简体中文。禁止繁体字（臺→台、體→体、爲→为、時→时、會→会、學→学、國→国、對→对、來→来、開→开、關→关、門→门、網→网、長→长、當→当、過→过、將→将、從→从、無→无、見→见、說→话、話→话、語→语、寫→写、讀→读、嗎→么、麼→们、們→个、個→为、為→应、應→经、經→处、處→风、風→东、東→麦、麥→卖）。违反此规则用户将无法使用。

你是手机通知摘要助手。任务:
1. 提取邮件中全部新闻条目(与兴趣标签无关，总数固定)
2. 逐一判断是否匹配兴趣标签，匹配→highlights，不匹配→other
3. highlights+other = 全部新闻，总数不受兴趣标签影响

匹配规则:
- 检查限定条件: 标签"内地生实习"但新闻"仅限澳门本地学生"→不匹配
- 标签"本科生奖学金"但"仅面向研究生"→不匹配
- 不确定时→放入other

每项必须提取: 截止时间(纯文本如"6月20日 上午10:00")、活动地点，填入deadline和location字段。
status字段也请填写你的判断，系统会用精确时间自动校正。summary/oneLiner中正常加【已结束】【进行中】【即将截止】等标记。
有原文链接必须填url。other每条独立列出，禁止"等等""等"省略。

风格: 推送通知式，title≤12字，summary/oneLiner≤40字。去废话留关键数字。

返回纯JSON(无markdown代码块):
{
  "highlights": [
    { "title": "奖学金申请", "summary": "截止: 6月20日，需提交成绩单【即将截止】", "matchedInterest": "奖学金", "url": "https://...", "status": "即将截止", "deadline": "6月20日", "location": "" }
  ],
  "other": [
    { "title": "图书馆闭馆", "oneLiner": "6月15-16日闭馆，地点: 主图书馆", "url": "https://...", "status": "", "deadline": "", "location": "主图书馆" }
  ]
}`;
...
function parseDeadline(str) {
  ...
  var month = -1, day = -1, hour = 23, minute = 59;

  // 匹配: "2024年6月20日"
  var mFull = str.match(/(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日/);
  if (mFull) {
    year = parseInt(mFull[1]);
    month = parseInt(mFull[2]) - 1;
    day = parseInt(mFull[3]);
  } else {
    // 匹配: "6月20日"
    var mSimple = str.match(/(\d{1,2})\s*月\s*(\d{1,2})\s*日/);
    if (mSimple) {
      month = parseInt(mSimple[1]) - 1;
      day = parseInt(mSimple[2]);
      // 如果月份比当前月份小很多(差6个月以上)，可能是明年
      if (month < now.getMonth() - 1) year = year + 1;
    } else {
      return null; // 无法解析
    }
  }

  // 匹配时间: "上午10:00", "下午3:00", "10:00", "下午3点"
  var mTime = str.match(/(上午|下午|早上|晚上)?\s*(\d{1,2})[:：](\d{2})/);
  if (!mTime) mTime = str.match(/(上午|下午|早上|晚上)?\s*(\d{1,2})\s*点/);
  if (mTime) {
    hour = parseInt(mTime[2]);
    minute = mTime[3] ? parseInt(mTime[3]) : 0;
    var ampm = mTime[1];
    if (ampm === '下午' || ampm === '晚上') {
      if (hour !== 12) hour += 12;
    } else if (ampm === '上午' || ampm === '早上') {
      if (hour === 12) hour = 0;
    }
  } else {
    // 只有日期没有时间，默认当天23:59:59
    hour = 23; minute = 59;
  }
...
function computeStatus(deadlineStr) {
  if (!deadlineStr || !deadlineStr.trim()) return { status: '', suffix: '' };
  var dl = parseDeadline(deadlineStr);
  if (!dl || isNaN(dl.getTime())) return { status: '', suffix: '' };
  var now = new Date();
  if (now > dl) return { status: '已结束', suffix: '【已结束】' };
  var diffDays = (dl - now) / (1000 * 60 * 60 * 24);
  if (diffDays <= 7) return { status: '即将截止', suffix: '【即将截止】' };
  return { status: '', suffix: '' };
}

function postProcessStatus(item) {
  if (!item) return;
  var computed = computeStatus(item.deadline);
  if (computed.status) {
    // 客户端精确计算成功 → 用精确结果覆盖
    item.status = computed.status;
    var text = item.summary || item.oneLiner || '';
    // 移除旧的AI状态标记，换精确的
    text = text.replace(/【[^】]*(?:已结束|进行中|即将截止)[^】]*】/g, '').trim();
    text = text + ' ' + computed.suffix;
    if (item.summary) item.summary = text.trim();
    else if (item.oneLiner) item.oneLiner = text.trim();
  }
  // 如果 deadline 无法解析 → 保留 AI 的 status (不覆盖)
}

function postProcessAllStatuses(result) {
  if (result.highlights) result.highlights.forEach(postProcessStatus);
  if (result.other) result.other.forEach(postProcessStatus);
}
...
async function callAI(emailContent, links) {
  ...
  const dateStr = `${now.getFullYear()}年${now.getMonth()+1}月${now.getDate()}日 ${hh}:${mm} (周${['日','一','二','三','四','五','六'][now.getDay()]})`;
  let userMessage = `现在时间: ${dateStr}\n兴趣标签: ${interestsStr}\n\n邮件内容:\n${emailContent}`;
...
async function analyze(silent) {
  ...
  // 截断邮件内容加速分析 (保留前2500字，足够覆盖所有新闻)
  const truncatedContent = emailContent.length > 2500
  ...
  try {
    const result = await callAI(truncatedContent, links);
