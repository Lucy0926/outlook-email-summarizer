...
status字段留空""，系统会自动精确计算。summary/oneLiner中不要加【已结束】【进行中】【即将截止】等标记。
...
function postProcessStatus(item) {
  if (!item) return;
  var result = computeStatus(item.deadline);
  if (result.status) {
    item.status = result.status;
    // 从 summary/oneLiner 中移除 AI 可能加的状态标记
    var text = item.summary || item.oneLiner || '';
    text = text.replace(/【[^】]*(?:已结束|进行中|即将截止)[^】]*】/g, '').trim();
    // 追加精确计算的状态
    text = text + ' ' + result.suffix;
